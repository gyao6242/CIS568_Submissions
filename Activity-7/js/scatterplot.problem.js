                                 ...
                                 margin = 50)
{
    ...
    let categoryVisible = {};
    colorCategories.forEach(c => categoryVisible[c] = true);

    legends_items.on("click", function(event, category) {
        categoryVisible[category] = !categoryVisible[category];

        d3.selectAll("circle")
            .filter(function(d){ return d[colorCol] === category })
            .classed("legendSelect", !categoryVisible[category]);  
        d3.select(this)
            .select("rect")
            .attr("opacity", categoryVisible[category] ? 1 : 0.5);
    });
